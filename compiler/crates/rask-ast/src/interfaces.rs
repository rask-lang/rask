...
//! Reading an interface name out of a type.
//!
//! `any Interface` is interface-object syntax (`type.generics/G7`), so the interface's name
...
//! interface-box cast. They have to agree on what the name is, which is the whole
//! reason this is one function rather than four `strip_prefix` calls.

/// Split `any Interface` into its interface name, or `None` if `s` isn't a
/// interface-object type.
pub fn interface_object_name(s: &str) -> Option<&str> {
    s.trim().strip_prefix("any ").map(str::trim)
}

/// Is this the short spelling of `any Error`?
///
/// `Error` on its own means the erased error box. It's the interface every error
...
mod tests {
    ...
    #[test]
    fn reads_the_interface_out_of_the_type() {
        assert_eq!(interface_object_name("any Error"), Some("Error"));
        assert_eq!(interface_object_name("any Displayable"), Some("Displayable"));
    }

    #[test]
    fn the_prefix_is_required() {
        assert_eq!(interface_object_name("Error"), None);
        assert_eq!(interface_object_name("anything"), None);
    }
