    ...
    fn default() -> Self {
        ...
        Self {
            codes: register_codes! {
                ...
                "E0327" => ("non-optional `Link<T>` field", Type,
                    "Deleting a node sets every edge pointing at it to `none`, so a scalar edge field has to be able to hold `none` — write it `Link<T>?`. A non-optional edge is also unbuildable: a cycle needs one side written before its target exists. Inside a container a bare link is fine, because delete drops the entry rather than nulling it.",
