        ...
        Self {
            codes: register_codes! {
                ...
                "E0328" => ("use after delete on a `Link<T>`", Type,
                    "A `Link<T>` is a pointer to a node, and `store.delete` frees that node — so every name for it dies at once. This is a use after free, proven at compile time rather than checked at runtime the way a handle is. A link held in a `Link<T>?` *field* survives a delete, because the store nulls it and the `?` makes you check; a local can't be reached by the store, so the compiler tracks it instead. Note the invalidation point is the `delete` you wrote, not an inferred last use — the analysis stays inside one function body.",
                    "let n = store.insert(Node { name: \"a\" })\nstore.delete(n)\nprintln(\"{n.name}\")   // error: `n` names a deleted node\n\n// fix: read before deleting\nlet label = n.name\nstore.delete(n)\nprintln(\"{label}\")"),
                ...
                "E0817" => ("invalid `as` cast", Type,
                    ...
                    "let x: i8 = big as i8  // error: use `big truncate to i8`"),
                "E0818" => ("invalid conversion form", Type,
                    ...
                    "let x = n float to int i32  // error if n is already an integer"),
                ...
                "E0825" => ("integer literal out of range", Type,
                    ...
                    "let b: u8 = 300  // error: 300 doesn't fit u8 (0..=255)\n// fix: `let b: u16 = 300`, or `let b = 300 truncate to u8`"),
