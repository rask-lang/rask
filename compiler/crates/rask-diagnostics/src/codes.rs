...
let old = a.load(Ordering.SeqCst)"),
                ...
                "E0893" => ("an operator\'s method declared outside its conformance", Type,
                    "`a * b` resolves on the pair of operand types against a declared conformance (type.operator-resolution/OR1), so a `mul` written in a plain `extend` block is a method wearing an operator\'s name with none of its meaning. It can\'t become the conformance later either: a type has one `mul` (type.generics/MN1), so the block that writes it is the block that has to say which pair it answers.\n\nThe twelve names belong to their traits — `add`, `sub`, `mul`, `div`, `rem`, `neg`, `bit_and`, `bit_or`, `bit_xor`, `bit_not`, `shl`, `shr`. Comparison is not among them: OR9 keeps `Equal` and `Comparable` same-type and off the pair, so an inherent `eq`, `lt` or `compare` is unaffected.\n\nIf the method isn\'t the operator — a buffer\'s `add`, say — the fix is a name that isn\'t taken. `push` is what the collections use.",
                    "extend Meters {\n    func mul(self, k: f64) -> Meters { … }   // error: `mul` is Mul\'s\n}\n// fix: say which pair\nextend Meters with Mul<f64> {\n    func mul(self, k: f64) -> Meters { … }\n}"),
