...
let old = a.load(Ordering.SeqCst)"),
                ...
                "E0889" => ("two conformances of one trait wanting different methods of the same name", Type,
                    "A type may conform to `Mul<f64>` and `Mul<Meters>` at once — they are separate conformances with separate signatures and separate associated types. What they cannot both have is the method: a type has one `mul` (type.generics/MN1), and two conformances asking for different ones leave `m.mul(x)` with nothing to resolve to.\n\nPicking between them from the argument\'s type is operator resolution — resolving on the pair rather than as a method lookup on the left operand — and that is a separate piece of work. `scoped extend` is MN4\'s answer to the same question and will be the escape hatch once it is implemented; for now, keep one conformance, or split the second onto a trait of its own so the two methods have different names.",
                    "extend Meters with Mul<f64> {\n    type Out = Meters\n    func mul(self, k: f64) -> Meters { … }\n}\nextend Meters with Mul<Meters> {     // error: two different `mul`s\n    type Out = SquareMeters\n    func mul(self, other: Meters) -> SquareMeters { … }\n}"),
