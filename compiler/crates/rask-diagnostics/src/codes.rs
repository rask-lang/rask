...
impl std::fmt::Display for ErrorCategory {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ErrorCategory::Interface => write!(f, "Interface"),
        ...
        Self {
            codes: register_codes! {
                ...
                "E0314" => ("infinite type", Type,
                    "Inference found a type variable inside its own binding — solving it would need a type that contains itself forever. Usually an annotation somewhere in the chain settles it. For a *declaration* that contains itself, see E0890.",
                    "// the checker cannot pin a type that would have to hold itself\n// fix: annotate the binding, or break the cycle with `Heap<T>`"),
                ...
                "E0317" => ("try with nowhere to go", Type,
                    "`try` leaves through the enclosing function's error branch, or, in a `test` block, ends the test (`std.testing/T20`). A constant's initializer has neither, so the error or `none` has to be handled where it happens.",
                    "const LIMIT: i64 = try parse_limit()        // error: nowhere to send the error\n// fix: say what the constant is when it fails\nconst LIMIT: i64 = parse_limit() ?? 100"),
                ...
                "E0322" => ("volatile view stored", Type,
                    "A view (reference) into a growable collection was stored across a statement boundary. Views into Vec and Map are instant — they're released at the semicolon.",
                ...
                "E0380" => ("a type that became a strategy", Type,
                    "`Cell` and `Mutex` used to be types of their own. They were one concept with `Shared` — one value, several accessors, a scoped view — differing only in what synchronization they took, so they became strategies on `Shared<T, S>` (analysis.storage-consolidation).",
                    "let c = Cell.new(0)          // error\nlet m = Mutex.new(0)         // error\n\n// fix:\nlet c = Shared.local(0)      // no lock, one task\nlet m = Shared.mutex(0)      // a plain lock"),
                "E0898" => ("a link sent to another task", Type,
                    "A `Link<T>` is its node's address, and any link can write the node. A task can't be handed one, directly or inside a struct, option or container, because two tasks would then write one node unordered. A whole `Rack` can move, since its links go with the nodes they name; `snapshot()` copies one for a task that needs to read while this one keeps writing (mem.ownership/T2, mem.racks).",
                    "let a = rack.insert(Node { id: 1 })\nlet h = spawn(|| { a.id += 1 })   // error\n\n// fix: copy out what the task needs\nlet id = a.id\nlet h = spawn(|| { use(id) })"),
                "E0897" => ("a blocking `Shared` access given a closure", Type,
                    "`read` and `write` on a `Shared` block until the lock is free, and a blocking access is scoped by a `with` block or by one expression (`s.read().field`). Closures are the shape of the non-blocking `try_read`/`try_write`, which can come back without the lock (conc.sync).",
                    "let d = s.read(|v| { return v * 2 })   // error\n\n// fix: a with block, whose value is its last expression\nlet d = with s.read() as v { v * 2 }"),
                ...
                "E0357" => ("single-letter type name", Type,
                    "Single uppercase letters are reserved for type parameters. A struct, enum, interface, or union named `T` would be shadowed by the type-parameter convention in every signature.",
                ...
                // Interface errors (E07xx)
                "E0700" => ("interface bound not satisfied", Interface,
                    "A generic function requires an interface bound that the provided type doesn't implement.",
                    "func print_it<T: Display>(x: T) { }\nprint_it(MyStruct {})  // error: MyStruct doesn't implement Display"),
                "E0701" => ("missing interface method", Interface,
                    "An `extend` block claims to implement an interface but doesn't define all required methods.",
                    "interface Printable { func show(self) }\nextend Point as Printable { }  // error: missing show()"),
                "E0702" => ("method signature mismatch", Interface,
                    "An interface method implementation has a different signature than the interface declaration.",
                    "interface Add { func add(self, other: Self) -> Self }\nextend Point as Add {\n    func add(self) -> Self { }  // error: wrong params\n}"),
                "E0703" => ("unknown interface", Interface,
                    "An interface name was used that doesn't exist. Check spelling and imports.",
                    "Point implements Printabel { }  // error: did you mean Printable?"),
                "E0704" => ("conflicting interface methods", Interface,
                    "Two interface implementations provide the same method name for a type. This creates ambiguity.",
                    "// Both InterfaceA and InterfaceB define show()"),
                ...
                "E0819" => ("index type mismatch", Type,
                    "An index expression `c[i]` used the wrong index type. Vec, arrays, slices, and strings are position-indexed by an integer; `Map<K,V>` is indexed by `K`. Range indexing (slicing) only works on Vec, arrays, slices, and strings.",
                    "let s = \"hi\"\nv[s]  // error: index a Vec with an integer, not a string"),
                "E0820" => ("linear value in container", Ownership,
                    "A Vec or Map element (or Map key) is a linear value — an @resource type, a transitively-linear struct/enum, or an optional/tuple/array built from one. Vec/Map drop can't consume linear elements, so they'd be silently dropped (RC1/RC3). A `Rack<T>` is out too (RC2): `delete` frees the node rather than handing it back, so nothing can consume one. `T?` (match and consume, RC4) is what's left.",
                    "@resource\nstruct File { fd: i32 }\nlet files: Vec<File> = Vec.new()  // error: hold it in a `File?` instead"),
                ...
                "E0824" => ("public duck interface", Type,
                    "A `duck interface` was declared `public`. Duck interfaces match by shape instead of by declaration, which makes them a versioning trap across a package boundary: an external type could start or stop satisfying the interface because its author added or removed a method, with nothing in either diff to notice. So duck interfaces stay package-internal (type.generics/DT1) — they're for code you're still sketching. Drop `duck` to harden the interface (the compiler generates the conformance declarations for types that already match), or drop `public`.",
                    "public duck interface Frobber {\n    func frobnicate(self)\n}\n// fix: `public interface Frobber` (nominal), or `duck interface Frobber` (package-internal)"),
                ...
                "E0826" => ("type does not implement Displayable", Type,
                    "`{}` in a format template calls `to_string()`, which comes from `Displayable` (std.fmt/D4). Primitives have it; structs and enums opt in with `Type implements Displayable`, and error types get it for free from `message()` (D5). Optionals and results are never Displayable — an optional may have nothing to show, so the missing case has to be spelled out at the call.",
                ...
                "E0827" => ("type can't be iterated", Type,
                    "A `for` loop walks a Vec, Map, array, slice, range or iterator chain. The thing in the iterator position resolved to a single value instead — most often a count where the range was meant, a string where `.chars()` was meant, or a struct where one of its collection fields was meant. A container reached through a field resolves later than the loop, so this is reported once its type settles rather than at the loop itself.",
                    "for x in self.count { … }   // error: `i64` can't be iterated\n// fix: `for x in 0..self.count { … }`"),
                "E0829" => ("with guard escapes its block", Type,
                    "A `with` block's guard — the name after `as` — is access to what the source holds, for the block's duration — not a value of its own. Rask hands out no guards, so the inner value can't outlive its scope. Returning the bare guard identifier as the block's own produced value would hand back a live view into memory the lock no longer protects once the block ends. This only fires for struct/enum/union payloads; scalars and `string` copy out fine as-is, since a plain identifier read of those is already an independent value.",
...
// fix: let v: Vec<i64> = [1, 2, 3]"),
                ...
                "E0834" => ("type can't be a Map key", Type,
                    ...
                    "type Id = u64\nlet ids: Map<Id, User> = Map.new()   // error: `Id` is not Hashable\n// fix: `type Id = u64 implements Equal, Hashable`"),
                "E0833" => ("no interface by that name", Type,
                    "A bound, a conformance header, or an `as any Interface` cast named something that isn\'t an interface. Usually a typo or a missing import. This used to be reported as \"`_` does not implement X\" — an unknown interface has no type to blame, so the placeholder stood in for one and the message pointed at the type system instead of at the name (type.generics/G1).",
                    "func narrow<T: Intger>(x: i64) -> T { … }   // error: no interface named `Intger`\n// fix: `func narrow<T: Integer>(x: i64) -> T { … }`"),
                ...
                "E0846" => ("`staged()` outside a `with` block", Type,
                    "Staged access takes the lock, binds a working copy, and commits it back as one move when the block exits — that exit is where the commit happens, so there has to be a block. `read` and `write` also have an expression-scoped form, `shared.write().field`, where the lock is held for just that chain; staged has no equivalent, because a single expression has no exit to commit at. Until this was rejected the interpreter reported an internal \"no method `staged`\" and native failed codegen with \"Function not found\".",
                ...
                "E0875" => ("a `comptime func` that reaches outside the compile-time subset", Type,
                    "CT7 rules out I/O, spawning and pool structural changes at compile time, and CT6 says a call in comptime position is legal iff the callee stays inside that subset — transitively, and with no marking required on the callee. `comptime func` is the assertion of that property at the definition: CT60 says the keyword buys definition-time checking, so the guarantee lands where the author is rather than erupting at a call site three packages away. Until this check existed the keyword bought nothing — a `comptime func` doing I/O compiled, and the failure came later and elsewhere, as the evaluator not finding a function it had never registered. The effect map is transitive, so a helper three hops down is named at the call that brings it in.",
                    "comptime func read_config() -> string {\n    return io.read_line() catch e => \"\"   // error: I/O isn't available while compiling\n}\n\n// fix: drop `comptime` and let callers decide\nfunc read_config() -> string { … }\n// or move the I/O out and pass the result in\ncomptime func parse_config(text: string) -> Config { … }"),
                "E0874" => ("a `with` source that is neither an element nor a `Shared`", Type,
                    "A `with` block scopes access to one thing: an element of a collection reached by index or key (`with items[i] as e`), or the value inside a `Shared` (`with shared.write() as v`). Both are cases where the block earns its keep — it holds the access open across several statements, and for a `Shared` holds the lock for the block's duration. A plain place is neither: there is nothing to hold, so `with h.data as d { d.push(1) }` does exactly what `h.data.push(1)` does, in three lines instead of one. Native compiled the form and the interpreter refused it at run time, which is how it went unnoticed — no file in the corpus uses it.",
                    "with h.data as d { d.push(1) }     // error: `h.data` is a `Vec`\n\n// fix: write through the field\nh.data.push(1)\n// or name an element\nwith h.data[0] as first { first.bump() }"),
                "E0873" => ("a writable second name for a field", Ownership,
                    "Reading a field produces a view that lives until the block ends (mem.borrowing/S1). A read-only view is fine — it sits beside a source that stays readable. `mut` asks for exclusive access, and S5 says a mutable borrow excludes all other access to the source; a plain binding has no way to say for how long, so both names stay live and a write through either is a write through both. `with` is the form that scopes the exclusion, `.clone()` the one that buys a separate value and shows the cost.",
                    "let h = Holder { data: Vec.new() }\nmut escaped = h.data      // error: `escaped` and `h.data` are one Vec\nescaped.push(1)           // ...and this puts an element in `h.data`\n\n// fix: reach it where it lives\nh.data.push(1)\n// or buy a copy, and the cost shows\nmut escaped = h.data.clone()"),
                "E0872" => ("a borrowed field returned to the caller", Ownership,
                    "A parameter without `take` is the caller's value on loan (mem.parameters/PM1), and reading a field of it produces a view that lives until the block ends (mem.borrowing/S1). S3 is the rule that says a view can't leave: returning one gives your caller a second name for storage the *first* caller still owns. For a `Vec` field that is literally the same buffer — a `push` through the returned value is a `push` into the struct, on both backends — and at the end whoever frees it second frees it twice. Return a copy, or declare `take` so the call site shows the value going.",
                    "extend Named {\n    func value(self) -> Vec<i64> {\n        return self.value       // error: hands out the caller's Vec\n    }\n}\n\n// fix: a copy, and the cost is visible\nfunc value(self) -> Vec<i64> { return self.value.clone() }\n\n// or say the value goes\nfunc into_value(take self) -> Vec<i64> { return self.value }"),
                "E0871" => ("a non-Copy element bound out of a collection", Ownership,
                    "Indexing gives access to the element where it lives — it doesn't copy it out (mem.borrowing/E1, E3). For a Copy element that distinction doesn't survive the binding: the value is copied and the two are independent. For anything else the binding is a second name for storage the collection still owns, and a write through either is a write through both, identically on the interpreter and native. E4 rejects it at the binding. `with` says the access is scoped and keeps the tie visible; `.clone()` says a separate value is wanted and shows the cost.",
                    "mut items = Vec<BigThing>.new()\nitems.push(BigThing { data: Vec.new() })\nlet x = items[0]        // error: `x` and `items[0]` are the same element\n\n// fix: scope it\nwith items[0] as x { use(x) }\n// or pay for a copy\nlet x = items[0].clone()"),
                "E0870" => ("two arguments of one call reach the same storage", Type,
                    "Borrows are tracked at field granularity (mem.borrowing/F1, F2), so `f(mutate p.health, p.score)` is fine — the two arguments name different fields and never meet. F3 is the case where one path contains the other: `f(mutate p.health, p)` gives the callee a write to one field and a reference to the whole struct beside it, and inside the body those are two names for overlapping memory with nothing in the signature saying so. Pass the fields the callee needs, or take the whole value once and reach the fields through it.",
                    "func both(mutate a: i32, b: Player) -> i32 { return b.score }\n\nlet n = both(mutate p.health, p)   // error: `p` contains `p.health`\n\n// fix: name the fields\nfunc both(mutate a: i32, b: i32) -> i32 { return b }\nlet n = both(mutate p.health, p.score)"),
                "E0840" => ("resource discarded as a statement", Ownership,
                    "A resource-typed value (marked @resource, like `Handle` — conc.async/H1) came back from a call used as a bare statement, with nothing to bind it to. The value is produced and dropped in the same instant, before anything could consume it — the same leak `E0805` catches for a named binding that falls out of scope unconsumed, just with no name to point at.",
                    "using Multitasking {\n    spawn(|| { work() })   // error: Handle dropped without join()/detach()\n}\n// fix: let h = spawn(|| { work() })\n//      h.detach()"),
                ...
                "E0332" => ("Self-returning method called through an interface object", Interface,
                    "`as any Interface` erases the concrete type, and a method returning `Self` has to name it — the caller would have no type to put the result in (type.interfaces/TR2). This is a property of the method, not of the call: the same method is fine on the concrete type, where `Self` is known. Split the interface if only some methods need erasing, or call this one before boxing.",
                    "interface Shape { func scaled(self, k: f64) -> Self }\nlet s: any Shape = circle\nlet bigger = s.scaled(2.0)   // error: what type is the result?\n// fix: scale first, then erase\nlet bigger: any Shape = circle.scaled(2.0)"),
                ...
                "E0808" => ("collection restructured inside its own `with` block", Ownership,
                    "A `with` block borrows one element in place, and `push`, `insert`, `remove` or a resize can move the whole buffer — which would leave the binding pointing at freed memory. Do the structural change outside the block. If the elements reference each other and get deleted individually, a `Rack` is the way: a node never moves, so a `Link` into one stays valid across anything the rack does.",
                ...
                "E0216" => ("dependency is out of scope for this build", Resolution,
                    "A dependency declared inside `scope \"dev\"` or `scope \"build\"` is linked by one kind of build and not the others. `\"dev\"` deps go where `test` blocks go: a release build strips the tests (`std.testing/T1`) and doesn't link the deps behind them, so code that ships can't reach one. `\"build\"` deps belong to `build.rk`.\n\nThe package does exist and the manifest does declare it — the build just isn't the one that links it. Either the use belongs in a test, or the dep doesn't belong in the scope.",
                    "scope \"dev\" { dep \"testkit\" { path: \"../testkit\" } }\nfunc main() { testkit.fixture() }   // error: out of scope\n// fix: move the dep out of `scope \"dev\"`, or keep the use in a test"),
                "E0217" => ("two tests with one name", Resolution,
                    "A test is picked by its name: `rask test -f` matches it, the report lists it, and `rask test --sim` prints a replay line naming it. Two tests (or two benchmarks) in one file with the same name can't be told apart by any of those, so the second is rejected (`std.testing/T1a`). It's nearly always a copied test whose name wasn't changed.",
                    "test \"parses\" { assert 1 == 1 }\ntest \"parses\" { assert 2 == 2 }   // error: two tests named \"parses\"\n// fix: name what each one checks\ntest \"parses one\" { assert 1 == 1 }\ntest \"parses two\" { assert 2 == 2 }"),
                ...
                "E0333" => ("type doesn't implement the interface a bound requires", Interface,
                    "A bound is a promise the caller has to keep. This type doesn't keep it — either it's missing the methods the interface declares, or the interface covers a fixed set of types (like the numeric ones) and this isn't one of them.\n\nConformance is nominal (#283): a type has an interface because an `T implements Interface` block says so, not because its methods happen to line up.",
...
let old = a.load(Ordering.SeqCst)"),
                "E0403" => ("a field and a method on one type share a name", Type,
                    "One name per member of a type (type.structs/M7). A field holding a function is callable — `h.run(5)` calls it (M6) — so a method of the same name would make `h.run` and `h.run(5)` reach two different things, with nothing in either line to say which. The declaration is rejected instead of a precedence rule being invented for the call sites.",
                    "struct Handler {\n    public run: func(i64) -> i64\n}\nextend Handler {\n    public func run(self, x: i64) -> i64 { … }   // error: `run` is already a field\n}\n// fix: rename one of them"),
                "E0405" => ("a field holding a function, called like a method", Type,
                    "`h.run(5)` resolves `run` against the type\'s extend blocks, and a field is not in one — so the call does not resolve even though the name is right there (type.structs/M6). Reading the field is fine; it is the one-step spelling that is missing.\n\nThere is no rule making the field callable, deliberately. A struct of functions is a shape Rask has a better answer for: an interface, and `any Handler` for the swappable case. Adding the spelling would be support for a shape the canon does not use, and it is much easier to add later than to remove once programs lean on it.",
                    "struct Handler { public run: func(i64) -> i64 }\nh.run(5)             // error: `run` is a field, not a method\n// fix: read it, then call it\nlet f = h.run\nf(5)"),
                "E0406" => ("ordering two links", Type,
                    "`Link<T>` compares for identity, never for order (mem.racks/RK11). A link is the address of its node (RK2), so `<` would answer from wherever the allocator put the chunk — pad the heap before the rack is built and the same program gives a different order, which is exactly the unreproducible failure determinism/D11 exists to rule out.\n\nThere is nothing honest to define here: \"node A is less than node B\" is a statement about malloc, not about the graph. Order by something the nodes declare instead. `==` and `!=` are unaffected — two links are equal when they name the same node, which is both meaningful and stable.",
                    "links.sort()                // error: no order over nodes\nif a < b { … }              // error: same\n// fix: order by a field the nodes declare\nlinks.sort_by_key(|l| l.id)\nif a.id < b.id { … }"),
                "E0408" => ("a type marked `@no_encode` or `@no_decode`", Type,
                    "The annotation is the type\'s own declaration saying its data does not belong in a wire format (std.encoding/E16). Its fields are usually perfectly serializable — a credential is a `string` — which is exactly why the rule has to be about the declaration and not about the fields.\n\nThe error fires wherever an `Encode` or `Decode` bound is asked of the type, which includes `json.encode(v)` with no written type argument.\n\nIt is separate from E0388, which means the fields genuinely have no wire representation. That one names the field; this one names the annotation, because there is no offending field to point at.",
                    "@no_encode\nstruct Secret {\n    public token: string\n}\n\njson.encode(s)        // error: `Secret` is marked `@no_encode`\n// fix: send a type that holds only what you meant to send\nstruct Claim {\n    public user: string\n}"),
                "E0410" => ("two packages declare the same conformance, and this code sees both", Type,
                    "One type conforms to one interface one way (type.generics/XC3). Two *packages* each declaring `Doc implements Labeled` is not either package\'s mistake — neither can see the other — so the error belongs where the two meet, which is code that needs the conformance and can reach both declarations.\n\nThat is also why a collision nobody uses costs nothing: pull in both libraries, never ask for `Doc: Labeled`, and the program builds. Visibility is the using package\'s, not the build\'s (XC4) — `liba` keeps using its own conformance even while the program linking it also pulls in `libb`\'s.\n\nThe way to have two behaviours is to have two types. A nominal type is one line, shares the layout, and carries a conformance of its own (XC6, type.aliases/T2, T13). To keep one of the two instead, move the code that needs it into a package that depends on one of them and not both.",
                    "// liba and libb each declare `interfacepkg.Doc implements Labeled`\nprintln(d.label())            // error: two conformances in scope\n// fix: a type of your own, and say what it does\ntype MyDoc = interfacepkg.Doc\nMyDoc implements Labeled { … }"),
                "E0409" => ("one of the six auto-derived interfaces, declared by a package that doesn\'t own the type", Type,
                    "Six interfaces belong to the package that declares the type (type.generics/XC1). Four of them — `Equal`, `Hashable`, `Comparable`, `Cloneable` — are one answer per type: `Map`, `Set` and every sort built on them assume the type answers the same way everywhere, so a second answer from another package doesn\'t conflict loudly, it makes lookups miss entries the container holds.\n\n`Encode` and `Decode` are the other two, and they are markers with no methods (std.encoding/E11). Declaring one doesn\'t change how a type serializes, it changes whether it does — so a third-party conformance overrules a `@no_encode` the owner wrote, which is E0408 answered from outside.\n\nA builtin has a declarer too — the standard library — so `Vec<i64> implements Hashable` or `string implements Comparable` in an ordinary program is the same error. Reading \"no package\" as \"nothing to check\" is what let that through, and those are the types a program actually puts in containers: `\"abc\".hash()` answered the program\'s number while every `Map` went on using the real one.\n\nEvery other interface is open: any package may conform any foreign type to any foreign interface (XC2), which is the point of the rules these six are carved out of. A conformance you want anyway goes on a nominal type of your own — one line, same layout, its own conformance (type.aliases/T2, T13).",
                    "// in `liba`, for a `Doc` that `interfacepkg` declares\ninterfacepkg.Doc implements Hashable { … }   // error: only `interfacepkg` can\n// fix: hash a type you own\ntype MyDoc = interfacepkg.Doc\nMyDoc implements Hashable { … }"),
                "E0407" => ("the same conformance declared twice", Type,
                    "One type conforms to one interface one way (type.generics/XC3). Two `T implements Interface` blocks both supply the interface's methods, and nothing in either block says which set wins — the answer used to be whichever the compiler read last, which meant reordering files changed what a program did.\n\nWithin a package this is reported at the second block, because you own both. Two packages declaring the same pair is the other half of XC3 and belongs at the use site, where the names of both packages are worth printing.\n\nThe way to have two behaviours is to have two types. A nominal type is one line, shares the layout, and carries a conformance of its own (type.aliases/T2, T13).",
                    "Doc implements Labeled { … }\nDoc implements Labeled { … }   // error: already declared\n// fix: one of them gets its own type\ntype ShortDoc = Doc\nShortDoc implements Labeled { … }"),
                "E0404" => ("`take` on a Copy type", Type,
                    "The three parameter modes are read at the call site to know what happens to the argument (mem.parameters/PM4, PM5). `take` says the caller loses the value — and a Copy value is never given away: it is copied into the parameter and the caller keeps theirs (PM6b). So `take` on a type that is Copy at every instantiation promises something that does not happen, and the rule is that a marker says the truth or is not written (PM6c).\n\nOnly the spelling is checked, and only for types whose Copy-ness cannot change: the primitives and `string`. `take p: Point` is fine — a struct stops being Copy the day it grows past sixteen bytes — and so is `take item: T`, which is how `Vec.push` is declared.",
                    "func charge(take amount: i64) -> i64 {   // error: `take` on `i64` takes nothing\n    return amount * 2\n}\n// fix: drop the keyword\nfunc charge(amount: i64) -> i64 { … }"),
                ...
                "E0862" => ("a closure holding a scoped borrow can't escape", Ownership,
                    "A closure that captures a block-scoped borrow lives as long as that block and no longer. Returning it, or storing it somewhere that outlives the block, would leave it holding a reference to something already gone.\n\nA closure that outlives its frame normally carries what it captured (mem.closures/CM1), so this only fires on a borrow it cannot take with it — one lent by a `with` block or a borrowed parameter. Give the closure a value of its own instead: clone what it needs, or hold the source somewhere that outlives it.",
                    "with data.read() as d {\n    return || { d.len() }    // error: closure would outlive the borrow\n}\n// fix: take a value the closure can keep\nlet n = with data.read() as d { d.len() }\nreturn || { n }"),
                "E0866" => ("a comptime field name that isn't a name", Type,
                    "`value.(expr)` reads the field `expr` names, so a `comptime` block in that position has to end with a string. A block that answers with anything else names no field.",
                    "b.(comptime { 42 })          // error: produces i64\n// fix: answer with the name\nb.(comptime { \"spent\" })"),
                "E0865" => ("a break value from a loop that produces none", Type,
                    "`while` and `for` are statements: when the condition goes false the loop ends with no value, so a `break` carrying one has nowhere to put it. `loop` is the form that produces a value — move the test inside it.",
                    "while cond {\n    break 42                 // error: `while` has no value\n}\n// fix: the test goes inside\nloop {\n    if !cond { break 42 }\n}"),
                "E0864" => ("a `match` on a type no set of arms can exhaust", Type,
                    "An integer, a float, a string or a char has more values than a list of arms can name, so a `match` on one needs an arm for whatever is left. Enums are different — their variants are finite, and covering them all is exhaustive without a wildcard.",
                    "match n {\n    1 => \"one\"\n    2 => \"two\"\n}                            // error: no arm for 3\n// fix: name the rest\nmatch n {\n    1 => \"one\"\n    2 => \"two\"\n    _ => \"many\"\n}"),
                "E0863" => ("`Error` is an interface, so it has no variants", Interface,
                    "`Error` is the interface every error type implements, and `any Error` is the erased form that holds one. Neither declares variants — those belong to the concrete error enum. Declare the error you mean and name its variant.",
                    "return Error.NotFound          // error: `Error` has no variants\n// fix: declare it\nenum LookupError { NotFound }\nreturn LookupError.NotFound"),
                "E0852" => ("a generic method can't be called through an interface object", Interface,
                    "`any Interface` erases the concrete type, and a generic method needs one — each instantiation is separate code, and there is nothing left to pick which. Call it on the concrete type instead (TR3).",
                ...
                "E0856" => ("package-level state written without a lock", Type,
                ...
                "E0878" => ("a variable two things want to write", Ownership,
                    "A closure that writes a variable it captured holds the only name for that variable while it lives. That is what makes the write safe with no lock and no annotation — the compiler can see there is nothing else to race with.\n\nA second closure over the same variable, or a read between two calls that change it, breaks that: two names for one piece of storage, and the answer depends on which runs first. mem.closures/MC2 is the rule; MC4 is why the capture ends at the closure's last use rather than at the end of the block, so reading the variable once the closure is done is exactly what you should do.",
                    "let a = || { n = n + 1 }\nlet b = || { n = n + 2 }      // error: `a` is still holding `n`\na()\nb()\n// fix: one closure, or a `Shared` they share\nlet n = Shared.new(0)\nlet a = || { with n.write() as v { v = v + 1 } }"),
                "E0880" => ("a field dropped by hand that its aggregate already owns", Ownership,
                    "Storing a `Heap` in a field moves it in. The aggregate's release hands it back when the aggregate dies — that is what makes a `Heap` field and a `Vec` field alike work without anything being written at the use site.\n\nA hand-drop of the same field is a second owner, and both of them run: the drop frees the block and the aggregate's release frees it again, which the allocator reports as a double free rather than as anything to do with the line that caused it.\n\nThe other way out would be for `drop` to quietly do nothing on a field, and that is worse — the author wrote a consume and got none, and whether `drop(x)` frees anything would depend on whether `x` is a binding or a projection.",
                    "struct Holder { inner: Heap<Big> }\nlet h = Holder { inner: Heap(Big { … }) }\ndrop(h.inner)                 // error: `h` gives it back already\n// fix: drop the whole thing, or just let it go out of scope\ndrop(h)"),
                "E0879" => ("a container's own value returned as if it were the caller's", Ownership,
                    "`m.get(k)` reads the value where the map keeps it — that is what makes a lookup cheap, and it is why the map still holds it afterwards. A function whose return type is `Vec<i64>` promises the caller something to own, so handing that lookup straight back gives two names to one buffer: the caller frees it, the map still points at it.\n\nThe shape that hides this is `??`. `m.get(k) ?? Vec.new()` is the map's vector on one path and a fresh one on the other, and the line doesn't say which. Nothing can be right for both, so it's rejected here rather than decided at runtime.\n\nThe fix is a copy, and the copy is written down: `get_clone` on `Vec` and `Map`, or `.clone()` on what a lookup answers with.",
                    "func lookup(index: Map<string, Vec<i64>>, word: string) -> Vec<i64> {\n    return index.get(word) ?? Vec.new()   // error: the map still holds it\n}\n// fix: hand back a copy\nfunc lookup(index: Map<string, Vec<i64>>, word: string) -> Vec<i64> {\n    return index.get_clone(word) ?? Vec.new()\n}"),
                "E0881" => ("a resource that would leak if a `try` fails", Ownership,
                    "A resource has to be consumed on every way out of its scope, and `try` is one of them: when the call it guards fails, the error goes straight to the caller and nothing in between runs. So a `try` between acquiring a resource and consuming it is a leak on the error path — invisible, because the happy path closes it and the tests take the happy path.\n\n`ensure` is the answer and the reason it exists: it commits the cleanup at the point of acquisition, so every exit from that scope runs it, including the ones written as `try`. Consuming the resource explicitly later cancels the ensure (mem.linear/L6), so the happy path still reads the way it did.",
                    "let file = try File.open(path)\nlet header = try file.read_header()   // error: `file` would leak if this fails\n// fix: commit the cleanup first\nlet file = try File.open(path)\nensure file.close()\nlet header = try file.read_header()"),
                "E0882" => ("a statement between acquiring a resource and committing its cleanup", Ownership,
                    "A linear value is owed a consumption from the moment it exists. Between the line that acquires it and the line that commits the cleanup, nothing is scheduled to close it — and a panic on any line in that gap takes the task down without running anything. There is no destructor behind it; the handle is simply gone.\n\nE0881 already refuses a `try` in that gap, because a failing `try` is an exit written in the source. A panic is the same leak through an exit nothing marks: an index, an overflow, a call that asserts. Rather than a house rule about keeping the gap short, the gap has to be empty — the statement after an acquisition commits it.\n\nCommitting means `ensure`, a consuming call, a move, or handing it back. `ensure` is the usual one, and consuming the value later cancels it (mem.linear/L6), so the happy path reads unchanged. Two values acquired together may take one statement each; each `ensure` shortens the window, and registering them in acquisition order is what makes the LIFO teardown come out right.",
                    "let file = try File.open(path)\nlet limit = config.read_limit()   // error: a panic here leaks `file`\nensure file.close()\n// fix: commit first, then do the work\nlet file = try File.open(path)\nensure file.close()\nlet limit = config.read_limit()"),
                "E0883" => ("cannot give away a const", Ownership,
                    "A `const` is one value the whole program reads (mem.ownership/O11). No function owns it, so none of them can hand it to a `take` parameter or store it into an aggregate — doing so would leave every other reader holding something that\'s gone.\n\nA Copy const never hits this: a scalar, a `string` or a small struct is copied into the `take` rather than given (mem.parameters/PM6b), and nothing is written at the call. What reaches here is a const that owns heap, where producing another one allocates — so the clone is written, like every other allocation.",
                    "const NAMES: Vec<string> = [\"a\", \"b\"]\ntake_it(NAMES)          // error: NAMES isn\'t yours to give\n// fix: hand over a copy\ntake_it(NAMES.clone())"),
                "E0884" => ("one expression takes two locks at once", Type,
                    "An inline `.read()`/`.write()`/`.lock()` holds its lock until the expression it starts ends (conc.sync/R5). Two of them in one expression therefore hold two locks at the same time, and two tasks that do that in opposite orders wait on each other forever (conc.sync/DL4). Give each access a statement of its own, or copy the value out with `.get()` first.",
                    "println(\"{s.read().theme} {s.read().size}\")   // error: two locks\n// fix:\nlet theme = s.read().theme\nlet size = s.read().size\nprintln(\"{theme} {size}\")"),
                "E0898" => ("two blocks defining the same method on one type", Type,
                    "A type has one method per name (type.generics/MN1). Two blocks each defining it — two `extend` blocks, or two conformances that both write a `label` — used to be accepted, with the block read last winning silently, so reordering two files changed what a program did.\n\nWhen two interfaces want the same method with the same signature, one implementation serves both (MN2): one block defines it, the other leaves it out. When they want different signatures, the second conformance goes on a nominal type of its own (MN3), which shares the bytes and carries its own conformances. That is a rule about one name, not about ownership; Rask has no orphan rule.",
                    "Doc implements Labeled {\n    func label(self) -> string { … }\n}\nDoc implements Tagged {\n    func label(self) -> i64 { … }      // error: `Doc` already defines `label`\n}\n// fix:\ntype TaggedDoc = Doc\nTaggedDoc implements Tagged {\n    func label(self) -> i64 { … }\n}"),
                "E0897" => ("a method called on an interface's name", Type,
                    "An interface names what a type can do. Its methods are ordinary methods of every type that implements it (type.generics/MN1), so they run on a value: `d.label()`, not `Labeled.label(d)`. There is no interface-qualified call; if two interfaces want the same method name on one type with different signatures, the second conformance goes on a nominal type of its own (MN3).",
                    "interface Labeled { func label(self) -> string }\nDoc implements Labeled { func label(self) -> string { … } }\n\nlet d = Doc { n: 1 }\nLabeled.label(d)      // error: `Labeled` is an interface\n// fix:\nd.label()"),
                "E0893" => ("a plain method inside an `implements` block", Type,
                    "An `implements` block is the contract. It holds the methods the interface declares (and the ones its parent interfaces declare), nothing else, so a reader can see exactly what the interface asks of the type by reading the block. A helper the interface never asked for is a plain method, and plain methods live in the type's own `extend T { }` block (type.generics/CD2).",
                    "interface Reader {\n    func read(mutate self, buf: Buffer) -> usize or IoError\n}\nLogSource implements Reader {\n    func read(mutate self, buf: Buffer) -> usize or IoError { … }\n    func rewind(mutate self) { … }        // error: `rewind` is not part of `Reader`\n}\n// fix:\nextend LogSource {\n    func rewind(mutate self) { … }\n}"),
                "E0889" => ("two conformances of one interface wanting different methods of the same name", Type,
                    "A type may conform to `Render<Html>` and `Render<Json>` at once — they are separate conformances with separate signatures and separate associated types. What they cannot both have is the method: a type has one `render` (type.generics/MN1), and two conformances asking for different ones leave `x.render(…)` with nothing to resolve to.\n\nThe operator interfaces are the exception, and the reason is that they have something else to go on. `Mul<f64>` and `Mul<Meters>` on one type are told apart by the argument — that is operator resolution (type.operator-resolution/OR1) — and each conformance\'s `mul` is filed under the argument it takes, so the two bodies keep separate symbols. Every other generic interface has only the name.\n\nFor the rest, the second conformance goes on a nominal type of its own: `type Loud = Doc` shares the bytes and carries its own conformances (type.aliases/T2, T13). This is a rule about one name, not about ownership — who declared the type or the interface never enters into it, and Rask has no orphan rule.",
                    "Report implements Render<Html> {\n    func render(self, f: Html) -> string { … }\n}\nReport implements Render<Json> {     // error: two different `render`s\n    func render(self, f: Json) -> Bytes { … }\n}"),
                "E0888" => ("a conformance method whose signature isn\'t the one the interface asked for", Type,
                    "The method is there; what it takes or answers with isn\'t what the interface declared. An interface with type parameters is the usual way to land here — `interface Scale<Rhs>` requires `scale(self, k: Rhs)`, and the conformance header decides what `Rhs` is, so `Meters implements Scale<f64>` needs `k: f64` and nothing else.\n\nThis used to be reported as a missing method, which sent people looking for a method that was right there.",
                    "interface Scale<Rhs> {\n    func scale(self, k: Rhs) -> Self\n}\nMeters implements Scale<f64> {\n    func scale(self, k: i64) -> Meters { … }   // error: Scale<f64> asks for `k: f64`\n}"),
                "E0885" => ("a generic interface used with the wrong number of arguments", Type,
                    "An interface's type parameter is substituted through every signature the interface requires, before any of them is checked. `interface Scale<Rhs> { func scale(self, k: Rhs) -> Self }` asks each conformance what `Rhs` is, and until it says, there is nothing to check an implementation against.\n\nA parameter declared with a default — `interface Mul<Rhs = Self>` — may be left out, and means the default. One without a default has to be written.",
                    "interface Scale<Rhs> {\n    func scale(self, k: Rhs) -> Self\n}\nMeters implements Scale { … }     // error: Scale takes 1 type argument, found 0\n// fix: say what it is\nMeters implements Scale<f64> { … }"),
                "E0886" => ("a conformance that leaves an associated type unanswered", Type,
                    "An associated type is the conformance's to name: the interface says `type Out` and each conformance says what its `Out` is. It is read off the conformance rather than inferred from the method bodies, which is what makes it a lookup with one answer instead of a search with several.\n\nThe interface can supply a default — `type Out = Self` — and then a conformance may stay silent. Without one, the block has to say.",
                    "interface Mul<Rhs> {\n    type Out\n    func mul(self, rhs: Rhs) -> Self.Out\n}\nMeters implements Mul<f64> {      // error: doesn\'t say what `Out` is\n    func mul(self, k: f64) -> Meters { … }\n}\n// fix:\nMeters implements Mul<f64> {\n    type Out = Meters\n    func mul(self, k: f64) -> Meters { … }\n}"),
                "E0887" => ("an associated type nobody declared", Type,
                    "`type X = …` in a conformance answers an associated type one of the block\'s interfaces declared, and `Self.X` in an interface signature names one that interface declares. A name that matches neither is either a typo or a member that was never added to the interface.",
                    "interface Mul<Rhs> {\n    type Out\n    func mul(self, rhs: Rhs) -> Self.Out\n}\nMeters implements Mul<f64> {\n    type Ouput = Meters            // error: no associated type `Ouput`\n}"),
                "E0895" => ("a literal operand more than one conformance could take", Type,
                    "A bare literal takes its type from what it is used with. `3 * duration` has nothing to take it from — the right operand isn\'t a number — so the type comes from the conformance instead: whichever primitive forms this pair with `Duration`.\n\nWhen more than one does, there is no one answer, and defaulting the literal would silently pick a pair the author didn\'t name. A suffix says which.",
                    "i64 implements Mul<Tick> { … }\ni32 implements Mul<Tick> { … }\nlet n = 3 * tick        // error: could be any of i64, i32\n// fix:\nlet n = 3i64 * tick"),
                "E0894" => ("an operator whose operand pair names no conformance", Type,
                    "`a * b` is resolved from both operand types, in order, against a conformance somebody declared (type.operator-resolution/OR1). Until `Meters implements Mul<f64>` exists, nothing says what `Meters * f64` means or what type it answers with — and the left operand having a method called `mul` isn\'t that: the header is what registers the pair.\n\nThe conformance may be written by a package that owns neither type, which is how `2.0 * meters` becomes writable at all.\n\nPrimitive pairs are the language\'s own — `i64 + i64` needs nothing declared, and a mismatched one is E0382 instead.",
                    "struct Meters { v: f64 }\nextend Meters {\n    func mul(self, k: f64) -> Meters { … }\n}\nlet scaled = m * 2.0     // error: no `*` between `Meters` and `f64`\n// fix: the same body, under a header\nMeters implements Mul<f64> {\n    func mul(self, k: f64) -> Meters { … }\n}"),
                "E0892" => ("an inherent method on a primitive", Type,
                    "`extend f64 { … }` adds nothing anyone can call: a primitive\'s methods come from a closed table in the compiler, and a block like this used to register nowhere at all — the method silently didn\'t exist, and the error turned up at the call site instead of at the block.\n\nA conformance is different. `f64 implements Mul<Meters>` registers against an interface, and the pair `(f64, Meters)` is what the operator resolves on — which is how `2.0 * meters` becomes writable by a package that owns neither type (type.operator-resolution/OR6).",
                    "extend f64 {                       // error: inherent method on a primitive\n    func doubled(self) -> f64 { … }\n}\n// fix: a conformance, on an interface that says what the operation is\nf64 implements Mul<Meters> {\n    type Out = Meters\n    func mul(self, m: Meters) -> Meters { … }\n}"),
                "E0890" => ("recursive type has no size", Type,
                    "A struct or enum reaches itself through inline storage only, so no finite layout exists: room for the value inside it means room for the one inside that. Fields, variant payloads, `T?`, tuples and arrays are all inline. `Heap<T>` puts the value in a block of its own and leaves the address in the field — the one for a list or a tree, where each node has a single owner. `Rack` plus `Link<T>?` is the one for nodes that point at each other or need identity. E0314 is the inference occurs-check, where a type variable turns up inside its own binding; this one is about a declaration.",
                    "struct Node {\n    value: i64\n    next: Node?    // error: `Node` contains itself\n}\n// fix: next: Heap<Node>?"),
                ...
                "E0899" => ("a part of a borrowed value given away", Ownership,
                    "Matching a value you only borrowed doesn't take it apart. The bindings in the arm are views into the caller's value, so they don't have to be consumed, and they can't be given away either — the same rule a borrowed parameter has (E0835).\n\nTake the value (`take s`) and the parts are yours: then each one has to be consumed.",
                    "func peek(s: Slot) -> i64 {\n    match s {\n        Slot.Empty => return 0,\n        Slot.Full(c) => { c.close()  return 1 }   // error\n    }\n}\n// fix: func finish(take s: Slot)"),
                "E0891" => ("a closure consumes what it only borrowed", Ownership,
                    "A plain closure borrows what it captures (mem.closures), and a borrow is not yours to give away — the same rule a borrowed parameter has (E0835). Consuming a captured `@resource` or `Heap<T>` inside the body would leave the outer binding holding something that is gone.\n\nThere is a second reason here the parameter case doesn't have: nothing says how many times a closure runs. One `close()` in the body reads as one consumption and is any number at runtime, so a closure handed to something that calls it twice closes the same handle twice — which native has no runtime flag to catch.\n\nHand the closure to something that declares `take` and it carries the resource in instead, so the closure owns it and the outer binding has nothing left to owe.",
