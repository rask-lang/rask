...
let old = a.load(Ordering.SeqCst)"),
                ...
                "E0864" => ("a `match` on a type no set of arms can exhaust", Type,
                    ...
                    "match n {\n    1 => \"one\"\n    2 => \"two\"\n}                            // error: no arm for 3\n// fix: name the rest\nmatch n {\n    1 => \"one\"\n    2 => \"two\"\n    _ => \"many\"\n}"),
                "E0863" => ("`Error` is a trait, so it has no variants", Trait,
                    "`Error` is the trait every error type implements, and `any Error` is the erased form that holds one. Neither declares variants — those belong to the concrete error enum. Declare the error you mean and name its variant.",
                ...
                "E0889" => ("two conformances of one trait wanting different methods of the same name", Type,
                    "A type may conform to `Render<Html>` and `Render<Json>` at once — they are separate conformances with separate signatures and separate associated types. What they cannot both have is the method: a type has one `render` (type.generics/MN1), and two conformances asking for different ones leave `x.render(…)` with nothing to resolve to.\n\nThe operator traits are the exception, and the reason is that they have something else to go on. `Mul<f64>` and `Mul<Meters>` on one type are told apart by the argument — that is operator resolution (type.operator-resolution/OR1) — and each conformance\'s `mul` is filed under the argument it takes, so the two bodies keep separate symbols. Every other generic trait has only the name.\n\n`scoped extend` is MN4\'s answer for the rest and will be the escape hatch once it is implemented; for now, keep one conformance, or split the second onto a trait of its own so the two methods have different names.",
                    "extend Report with Render<Html> {\n    func render(self, f: Html) -> string { … }\n}\nextend Report with Render<Json> {     // error: two different `render`s\n    func render(self, f: Json) -> Bytes { … }\n}"),
                "E0888" => ("a conformance method whose signature isn\'t the one the trait asked for", Type,
                    "The method is there; what it takes or answers with isn\'t what the trait declared. A trait with type parameters is the usual way to land here — `trait Scale<Rhs>` requires `scale(self, k: Rhs)`, and the conformance header decides what `Rhs` is, so `extend Meters with Scale<f64>` needs `k: f64` and nothing else.\n\nThis used to be reported as a missing method, which sent people looking for a method that was right there.",
                    "trait Scale<Rhs> {\n    func scale(self, k: Rhs) -> Self\n}\nextend Meters with Scale<f64> {\n    func scale(self, k: i64) -> Meters { … }   // error: Scale<f64> asks for `k: f64`\n}"),
                "E0885" => ("a generic trait used with the wrong number of arguments", Type,
                    "A trait's type parameter is substituted through every signature the trait requires, before any of them is checked. `trait Scale<Rhs> { func scale(self, k: Rhs) -> Self }` asks each conformance what `Rhs` is, and until it says, there is nothing to check an implementation against.\n\nA parameter declared with a default — `trait Mul<Rhs = Self>` — may be left out, and means the default. One without a default has to be written.",
                    "trait Scale<Rhs> {\n    func scale(self, k: Rhs) -> Self\n}\nextend Meters with Scale { … }     // error: Scale takes 1 type argument, found 0\n// fix: say what it is\nextend Meters with Scale<f64> { … }"),
                "E0886" => ("a conformance that leaves an associated type unanswered", Type,
                    "An associated type is the conformance's to name: the trait says `type Out` and each conformance says what its `Out` is. It is read off the conformance rather than inferred from the method bodies, which is what makes it a lookup with one answer instead of a search with several.\n\nThe trait can supply a default — `type Out = Self` — and then a conformance may stay silent. Without one, the block has to say.",
                    "trait Mul<Rhs> {\n    type Out\n    func mul(self, rhs: Rhs) -> Self.Out\n}\nextend Meters with Mul<f64> {      // error: doesn\'t say what `Out` is\n    func mul(self, k: f64) -> Meters { … }\n}\n// fix:\nextend Meters with Mul<f64> {\n    type Out = Meters\n    func mul(self, k: f64) -> Meters { … }\n}"),
                "E0887" => ("an associated type nobody declared", Type,
                    "`type X = …` in a conformance answers an associated type one of the block\'s traits declared, and `Self.X` in a trait signature names one that trait declares. A name that matches neither is either a typo or a member that was never added to the trait.",
