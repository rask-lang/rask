...
//! TD2: an interface method with a body is inherited by every conformer that doesn't
//! write its own.
//!
...
/// bodies fresh NodeIds — a clone carries the interface's own ids, and `node_types`
...
pub(crate) fn inject(decls: &mut [Decl]) -> Injected {
    ...
    // whole type, not just the block that names the interface.
    ...
    for (decl_index, decl) in decls.iter_mut().enumerate() {
        ...
        for interface_name in &claimed {
            let Some((_, defaults)) = interfaces.get(interface_name) else { continue };
