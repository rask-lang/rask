        ...
        .filter_map(|stmt| match &stmt.kind {
            MirStmtKind::LoadCapture { dst, offset, .. } => {
                let ty = func.locals.iter()
                    .find(|l| l.id == *dst)
                    .map(|l| l.ty.clone())
                    .unwrap_or_else(|| crate::fallback::unknown_type("transform/state_machine:191"));
                Some(CaptureInfo {
                    dst_local: *dst,
                    env_offset: *offset,
                    ty,
...
fn build_state_layout(
    ...
    for local_id in live_locals {
        ...
        let ty = local.map(|l| l.ty.clone()).unwrap_or_else(|| crate::fallback::unknown_type("transform/state_machine:332"));
        let size = ty.size();
        let align = ty.align();
        offset = align_up(offset, align);
...
fn remap_stmt(
    ...
    if let MirStmtKind::LoadCapture { dst, env_ptr, offset, access } = &stmt.kind {
    ...
    let kind = match &stmt.kind {
        ...
        MirStmtKind::LoadCapture { dst, env_ptr, offset, access } => MirStmtKind::LoadCapture {
            dst: remap_id(*dst, map),
        ...
        MirStmtKind::ResourceRegister { dst, type_name, slot } => {
