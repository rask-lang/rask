...
use super::HiddenParamPass;

/// Phase 2: Build the call graph from function bodies.
///
/// Callee edges are keyed by the type checker's recorded dispatch target
/// (`callee_key`), so `recv.m()` resolves to the same "Type.method" key the
/// declaration is stored under — instance-method propagation just works.
pub fn build_call_graph(pass: &mut HiddenParamPass, decls: &[Decl]) {
    let mut edges: Vec<(String, HashSet<String>)> = Vec::new();
    for decl in decls {
        match &decl.kind {
            DeclKind::Fn(f) => {
                let callees = collect_callees_from_body(pass, &f.body);
                if !callees.is_empty() {
                    edges.push((f.name.clone(), callees));
                }
            }
            DeclKind::Struct(s) => {
    ...
    for (caller, callees) in edges {
        pass.call_graph.insert(caller, callees);
    }
}
...
fn collect_callees_from_expr(pass: &HiddenParamPass, expr: &Expr, callees: &mut HashSet<String>) {
    match &expr.kind {
        ExprKind::Call { func, args } => {
            // Key by the recorded dispatch target, not a name mangled from `func`.
            if let Some(name) = pass.callee_key(expr.id) {
                callees.insert(name);
            }
