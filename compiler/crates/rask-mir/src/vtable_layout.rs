...
//!
//! Lowering picks the offset for a `InterfaceCall` and codegen writes the method
...
//! `InterfaceCall` reading one word past its method — a segfault on the first
//! dispatch.
//!
...
//! release for them; a box built for a call does not, and `InterfaceDrop` (which is
...
/// Byte offset of the method at `index` among an interface's compatible methods.
