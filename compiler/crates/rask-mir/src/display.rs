...
impl fmt::Display for BinOp {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let sym = match self {
            ...
            BinOp::Shl => "<<",
            BinOp::Shr => ">>",
            BinOp::WrappingAdd => "wrapping_add",
            BinOp::WrappingSub => "wrapping_sub",
            BinOp::WrappingMul => "wrapping_mul",
            BinOp::WrappingShl => "wrapping_shl",
            BinOp::WrappingShr => "wrapping_shr",
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.kind {
            ...
            MirStmtKind::ResourceRegister { dst, type_name, slot } => {
                match slot {
                    Some(l) => write!(f, "_{} = resource_register({} in _{})",
                                      dst.0, type_name, l.0),
                    None => write!(f, "_{} = resource_register({})",
                                   dst.0, type_name),
                }
            }
