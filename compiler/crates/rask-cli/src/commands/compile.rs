...
pub fn extract_tests(decls: &mut Vec<Decl>, filter: Option<&str>) -> Vec<(String, String)> {
    ...
    // Generate a synthetic main() that calls each __test_N() so monomorphization
    // marks them reachable. The real entry point (rask_main) is generated by codegen.
    if !tests.is_empty() {
        let dummy_span = Span { start: 0, end: 0 };
        let mut body: Vec<Stmt> = tests.iter().map(|(_, fn_name)| {
            Stmt {
                id: NodeId(0),
...
/// Compile tests to a native object file.
///
/// Like `compile_to_object` but transforms test blocks into functions
/// and generates a runner entry point that calls `rask_test_run` for each.
pub fn compile_tests_to_object(
    mono: &MonoProgram,
    typed: &rask_types::TypedProgram,
    decls: &[Decl],
    comptime_globals: &HashMap<String, rask_mir::ComptimeGlobalMeta>,
    ...
    if mir_functions.is_empty() && tests.is_empty() {
        return Err(vec!["no functions or tests to compile".to_string()]);
    }
