...
impl Interpreter {
    ...
    /// Hand every live resource in `value` over to a task's interpreter.
    ///
    /// A task runs on its own `Interpreter`, so it has its own resource
    /// tracker. An `own` capture moves the resource into the task — the
    /// ownership pass says so at compile time — but the parent's tracker
    /// never heard, so it went on owing a resource the task had already
    /// closed and reported a leak at scope exit for a program that ran
    /// correctly. Native, with no tracker, printed nothing. Same walk as
    /// `transfer_resource_to_scope`, moving entries out instead of re-depthing
    /// them (#882).
    pub(crate) fn hand_resources_to_task(&mut self, value: &Value, child: &mut Interpreter) {
        if self.resource_tracker.is_empty() {
            return;
        }
        let mut move_one = |this: &mut Self, id: u64| {
            if let Some(entry) = this.resource_tracker.take_entry(id) {
                child.resource_tracker.insert_entry(id, entry, 0);
            }
        };
        match value {
            Value::File(rc) => {
            ...
            Value::TaskHandle(h) | Value::ThreadHandle(h) => {
                ...
                if let Some(id) = self.resource_tracker.lookup_handle_id(ptr) {
                    move_one(self, id);
                    child.resource_tracker.register_handle_id(ptr, id);
                }
            }
            Value::Struct(ref s) => {
                let (id, fields) = {
                ...
                for field in &fields {
                    self.hand_resources_to_task(field, child);
                }
            }
            Value::Enum { fields, .. } => {
                for field in fields {
            ...
            Value::Tuple(items) => {
                for item in items.iter() {
                    self.hand_resources_to_task(item, child);
                }
            }
            Value::Vec(items) => {
