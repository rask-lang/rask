...
impl Interpreter {
    ...
    pub(crate) fn call_string_method(
        ...
        match method {
            ...
            // NFC (U5). ASCII is already normal; the tables for the rest
            // aren't built yet, so this returns the input rather than
            // corrupting it. See specs/stdlib/strings.md implementation notes.
            "normalized" => {
                Ok(Value::String(Arc::new(Mutex::new(s.lock().unwrap().clone()))))
            }
...
//
// These mirror the range tables in compiler/runtime/string.c. The interpreter
// is the reference for what the answer should be, so the two have to agree —
// keep them in step when either changes.

/// Combining marks, ZWJ, variation selectors: rendered on top of the previous
/// character rather than beside it.
fn is_zero_width(c: char) -> bool {
    ...
    matches!(c,
        ...
        | 0x09D7 | 0x09E2..=0x09E3 | 0x0A01..=0x0A03 | 0x0A3C..=0x0A51
        | 0x0A70..=0x0A71 | 0x0A75 | 0x0A81..=0x0A83 | 0x0ABC..=0x0ACD
        | 0x0AE2..=0x0AE3 | 0x0B01..=0x0B03 | 0x0B3C..=0x0B57 | 0x0B82
        | 0x0BBE..=0x0BCD | 0x0BD7 | 0x0C00..=0x0C04 | 0x0C3E..=0x0C56
        | 0x0C81..=0x0C83 | 0x0CBC..=0x0CD6 | 0x0D00..=0x0D03
        | 0x0D3B..=0x0D4D | 0x0D57 | 0x0D82..=0x0D83 | 0x0DCA..=0x0DDF
        | 0x0E31 | 0x0E34..=0x0E3A | 0x0E47..=0x0E4E | 0x0EB1
        | 0x0EB4..=0x0EBC | 0x0EC8..=0x0ECD | 0x0F18..=0x0F19 | 0x0F35
        | 0x0F37 | 0x0F39 | 0x0F71..=0x0F84 | 0x0F86..=0x0F87
        | 0x0F8D..=0x0FBC | 0x0FC6 | 0x102B..=0x103E | 0x1056..=0x1059
        | 0x105E..=0x1060 | 0x1062..=0x1064 | 0x1067..=0x106D
        | 0x1071..=0x1074 | 0x1082..=0x108D | 0x108F | 0x109A..=0x109D
        | 0x135D..=0x135F | 0x1712..=0x1714 | 0x1732..=0x1734
        | 0x1752..=0x1753 | 0x1772..=0x1773 | 0x17B4..=0x17D3 | 0x17DD
        | 0x180B..=0x180D | 0x1885..=0x1886 | 0x18A9 | 0x1920..=0x193B
        | 0x1A17..=0x1A1B | 0x1A55..=0x1A7F | 0x1AB0..=0x1AFF
        ...
        | 0x1CF4 | 0x1CF7..=0x1CF9 | 0x1DC0..=0x1DFF | 0x200B..=0x200F
        ...
        | 0xA806 | 0xA80B | 0xA823..=0xA827 | 0xA880..=0xA881
        | 0xA8B4..=0xA8C5 | 0xA8E0..=0xA8F1 | 0xA926..=0xA92D
        | 0xA947..=0xA953 | 0xA980..=0xA983 | 0xA9B3..=0xA9C0
        | 0xAA29..=0xAA36 | 0xAA43 | 0xAA4C..=0xAA4D | 0xAAB0
        | 0xAAB2..=0xAAB4 | 0xAAB7..=0xAAB8 | 0xAABE..=0xAABF | 0xAAC1
        | 0xAAEB..=0xAAEF | 0xAAF5..=0xAAF6 | 0xABE3..=0xABEA
        | 0xABEC..=0xABED | 0xFB1E | 0xFE00..=0xFE0F | 0xFE20..=0xFE2F
        | 0xFEFF | 0xFFF9..=0xFFFB | 0x101FD | 0x10376..=0x1037A
...
fn is_wide(c: char) -> bool {
    ...
    matches!(c,
        ...
        | 0x2648..=0x2653 | 0x267F | 0x2693 | 0x26A1 | 0x26AA..=0x26AB
        | 0x26BD..=0x26BE | 0x26C4..=0x26C5 | 0x26CE | 0x26D4 | 0x26EA
        | 0x26F2..=0x26F3 | 0x26F5 | 0x26FA | 0x26FD | 0x2705
        | 0x270A..=0x270B | 0x2728 | 0x274C | 0x274E | 0x2753..=0x2755
        | 0x2757 | 0x2795..=0x2797 | 0x27B0 | 0x27BF | 0x2B1B..=0x2B1C
        | 0x2B50 | 0x2B55 | 0x2E80..=0x303E | 0x3041..=0x33FF
        ...
        | 0x1F947..=0x1F9FF | 0x1FA70..=0x1FAFF | 0x20000..=0x2FFFD
        | 0x30000..=0x3FFFD)
}

/// Columns this scalar occupies. Control characters are zero — emitting them
/// is the caller's problem, not the width's.
fn scalar_width(c: char) -> usize {
    let n = c as u32;
    if n == 0 || n < 0x20 || (0x7F..0xA0).contains(&n) {
        0
    } else if n < 0x7F {
        1
...
/// Display columns for a whole string (U2). ASCII is the printable byte count
/// (U4) — the common case never touches a table.
...
/// Split into grapheme clusters (UAX #29, the rules that matter in practice):
/// CRLF stays together, combining marks and variation selectors join what
/// precedes them, ZWJ glues emoji sequences, regional indicators pair into one
/// flag.
pub(crate) fn graphemes(s: &str) -> Vec<String> {
    ...
    while i < chars.len() {
        ...
        if first == '\r' && i < chars.len() && chars[i] == '\n' {
            out.push(chars[start..i + 1].iter().collect());
            i += 1;
            continue;
        }
        if is_regional_indicator(first) && i < chars.len() && is_regional_indicator(chars[i]) {
            i += 1;
        }
        while i < chars.len() {
            let c = chars[i];
...
pub(crate) fn truncate_to_width(s: &str, cols: i64) -> String {
    ...
    for g in graphemes(s) {
        let w: usize = g.chars().map(scalar_width).sum();
        if used + w > cols {
            break;
        }
        used += w;
