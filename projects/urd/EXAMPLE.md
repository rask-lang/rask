...
7. **The `filter(|t| ...)` calls need non-escaping closures** — Raido specs function references only. The resolution (see [RESEARCH.md](RESEARCH.md) stress-test findings): lambdas capture freely but are second-class — argument position only, never stored or returned — so captures stay on the stack, nothing new serializes, and the compiler can inline them. Escaping closures stay cut: a closure stored in state would pin bytecode inside data and wreck the upgrade story.
