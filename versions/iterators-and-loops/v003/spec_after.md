...
| Collection | Index/Handle Mode | Ref Mode | Drain Mode |
|------------|-------------------|----------|------------|
| `Vec<T>` | `for i in vec` → `usize` | N/A | `for item in vec.drain()` → `T` |
| `Pool<T>` | `for h in pool` → `Handle<T>` | `for (h, item) in &pool` → `(Handle<T>, &T)` | `for item in pool.drain()` → `T` |
| `Map<K,V>` | `for k in map` → `K` (K: Copy) | `for (k, v) in &map` → `(&K, &V)` | `for (k,v) in map.drain()` → `(K, V)` |

**Vec iteration:**
- Index mode only (ref mode not needed—indices are cheap)
- Drain mode for ownership transfer

**Pool iteration:**
- Handle mode for mutations/removals
- Ref mode for ergonomic read access (both handle AND data)
- Drain mode for consumption
...
**Ref mode borrows:** Despite the `&` syntax, ref mode does NOT create a collection-level borrow. Each iteration yields expression-scoped refs. Between iterations, no borrow is active. However, mutations are forbidden to prevent invalidation.
...
| Drain | Consuming all items, transferring ownership |
