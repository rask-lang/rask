...
| [non_optional_link.rk](non_optional_link.rk) | A scalar edge must be `Link<T>?` — a bare `Link<T>` field has no `none` for delete to set, and can't be built under a cycle (E0327). Bare links inside `Vec`/`Map` stay legal |
