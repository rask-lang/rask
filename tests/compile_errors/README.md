...
| [store_link_use_after_delete.rk](store_link_use_after_delete.rk) | Using a `Link<T>` after `store.delete` freed its node (E0328) — reported as a use after free, not a move — a read, a write, and `contains`. The locals rule: a non-optional link asserts its node is alive, so a delete makes the use contradict the type |
