...
| [consume_borrowed_param.rk](consume_borrowed_param.rk) | Giving away a parameter the caller only lent (PM1/L1, E0835, #804) — a `take self` method, a `take` parameter, and `own` at the call site; `take` on the declaration is the way to say it |
