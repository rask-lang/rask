...
# The verdict is memcheck's own output, not the exit code.
#
# --error-exitcode only applies when memcheck gets to choose the exit code, and
# on the findings that matter most it doesn't: a program that reads freed memory
# and then dies from the resulting SIGSEGV exits 139, because the signal killed
# it before valgrind could substitute anything. The first cut of this gate
# checked for 42 and counted every one of those as clean — a gate that goes
# quiet on exactly the loudest half of what it measures.
#
# With --quiet, memcheck prints nothing unless it found something, so any
# `==pid==` line on its channel is a finding. The exit code is kept only to tell
# a clean run from a program that failed for its own reasons.
VG_EXIT=42
...
check_one() {
  ...
  exec 3>&2 2> /dev/null
  valgrind --error-exitcode=$VG_EXIT --leak-check=no --errors-for-leak-kinds=none \
...
for file in "$SUITE"/*.rk; do
  ...
  if [ -n "$detail" ]; then
    ...
    # Memcheck found nothing but the program didn't finish: a panic, an abort, a
    # stack overflow. The differential harness owns test failures, so this
    # doesn't fail the gate — but a program that stopped early is a program
    # whose later code nobody measured, and calling that clean is how a gate
    # quietly shrinks.
    broken=$((broken + 1))
...
echo "──────────────────────────────────────────────────"
echo "memcheck gate: $green clean, $expected known-bad, $bad new, $broken not measured"
