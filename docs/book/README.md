...
`rask test-specs docs/book/src` runs them all.

`tests/book_gate.sh` enforces this. It also checks that every include resolves — file
present *and* the named anchor really in it, because mdBook renders a missing anchor as
...
[specs/RULINGS.md](../../specs/RULINGS.md) is where those arguments come from. If a
...
[specs/DAY_ONE.md](../../specs/DAY_ONE.md) holds the reading set: the concepts you need
