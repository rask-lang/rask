...
- [ ] **MIR lowering: module-level constants not visible** — `const BLOOM_BITS: i32 = 256` etc. at file scope cause `UnresolvedVariable` during MIR lowering. Affects all file-scoped constants in multi-file packages: `BLOOM_BITS`, `MEMTABLE_FLUSH_THRESHOLD`, `MAX_LEVELS`, `MAX_CACHE_ENTRIES`, `BLOCK_SIZE`, `BLOOM_HASH_COUNT`. Constants need to be propagated into MIR scope.
- [ ] **MIR lowering: `i64` as variable** — `BlockCache_evict_one` reports `UnresolvedVariable("i64")`. Likely a `as i64` cast or type annotation parsed as a variable reference during MIR lowering.
- [ ] **MIR lowering: string interpolation variable capture** — `send_request` and `format_http_request` report `UnresolvedVariable("value")`. Likely string interpolation `"{value}"` not resolving the captured variable in MIR.
