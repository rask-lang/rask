...
>
> **And a third (2026-08-06): the 08-03 split is itself reversed.** `??` is retired entirely; the fallback is `orelse` on **both** shapes, its right side a value or a written-out divergence; `try` is back on both shapes too, propagating `none` from a `T?` exactly as it propagates the error from a `T or E` (the 08-03 restriction survives only where it was ever real: bare `try` on a flat `T? or E` operand is a compile error, resolved by `try f() orelse …`). The unification argument this proposal made is what carried the reversal — since `T?` *is* `T or none`, "the payload or this instead" is one operation, and the census over the tree plus expert Rust corpora showed the split words were tracking a distinction with no semantic content. `?`, `?.`, `x? as v`, `is none` remain absence-only; they're the surface that genuinely differs. Normative: `type.errors/ER14`, ER16, ER47; `type.optionals/OPT11`.
