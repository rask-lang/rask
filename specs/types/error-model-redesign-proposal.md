...
> **Superseded again (2026-08-06): two words across both shapes — `try` and `orelse`.** The 08-03 revision below split the fallback per shape (`??` vs value-`or`) and routed every exit through `try`'s `else` clause. Both halves reversed once fallback was recognised as the *same operation* on both shapes and the clause as a second propagation form earning nothing. Current surface: `try x` propagates the bad branch of either shape (`none` from a `T?`, the error from a `T or E`; bare `try` on a flat `T? or E` operand is a compile error — write `try f() orelse …`, ER16b); `x orelse <expr>` supplies the other branch on either shape, and its right side may be a value **or any divergence**, written where it happens (`orelse return E`, `orelse break`); `orelse e =>` binds the error (results only). `??`, value-level `or`, and the whole `try … else` clause family (ER45/ER46/ER48) are gone. `catch` was evaluated for the fallback word and rejected — it pairs with `throw` in every language a reader knows, and Rask has no `throw` (same reasoning as item 2 below). Normative: [error-types.md](error-types.md) ER12–ER18, ER44, ER47; [optionals.md](optionals.md) OPT11. Translating the 08-03 forms: `x ?? v` / `r or v` → `x orelse v`; `try x else return E` → `x orelse return E`; `try r else e => return f(e)` → `r orelse e => return f(e)`; `?? return none` → bare `try x`; `r or none` → `r orelse none`.
>
