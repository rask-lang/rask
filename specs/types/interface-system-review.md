<!-- id: type.interface-review -->
<!-- status: folded -->
<!-- summary: Historical — scenario-driven review of the interface system after the nominal flip. All rules folded into generics.md, gradual-constraints.md, error-types.md, structs.md, SYNTAX.md. Kept as the decision record. Post-fold amendment: condition inference for conditional conformance (this file's CC2) was deferred — explicit where required everywhere for now, inference is an additive relaxation later -->
<!-- depends: types/generics.md, types/interfaces.md, types/gradual-constraints.md -->

# Interface System Review
...
## How interfaces actually get used
...
| 9 | Conform a generic container to an interface | Rare but library-critical | ✗ no rule |
| 10 | Two interfaces sharing a method name | Rare | ✗ no rule |
...
## Finding 1: Overriding one auto-derived interface silently breaks its family

**Scenario 8.** The Equal/Hashable/Comparable family carries cross-interface contracts: `a == b` implies `hash(a) == hash(b)`, and `compare` must agree with `eq`. Auto-derive keeps them consistent by construction. Overriding *one* breaks the others silently:
...
This is the same corruption class the #312 core-interface carve-out exists to prevent, arising *within* one package. Pre-flip this couldn't happen — there were no declared overrides.
...
**Scenario 9.** `Stack<T>` is Displayable only when `T` is. Auto-derive handles this for the core five (a `Vec<T>` of Cloneable is Cloneable), which hides the gap: user interfaces on user generics have no rule at all. Structural matching used to give this for free — the shape either compiled per-instantiation or didn't.
...
**Scenarios 4→5, the prototype-to-production pipeline.** Gradual constraints deliberately keep private inference structural (GC3 collects method-requirements, not interface names). That's the right call — prototyping glue, invisible in APIs. But the promotion step got three unexamined edges:
...
2. **Promotion can hit a wall.** "Make public" must name interfaces. If the body's method-requirements match no interface, there is nothing to name — the honest fix is defining an interface, and the tooling should say so instead of pretending a signature exists. If they match *two* interfaces, promotion is ambiguous.
3. **The GC5 error message oversells.** It displays an inferred signature with a named interface (`<T: Validatable>`) recovered from shape — the exact resolution the flip removed. Display must be honest about which case it's in.
...
| **IS2: Promotion is exact** | "Make public"/"make explicit" fills in a named interface only when exactly one visible interface covers the residual method-requirements. Zero matches: report the methods and offer to generate an interface definition. Two+: list candidates, user picks |
...
### Prototyping with interfaces: the dial already exists
...
|-------|----------------|------------------------|
| Sketching | `duck interface Frobber { ... }` | Nothing — shape matches |
| Hardening | delete `duck` | Accept the generated `extend ... with` lines |
| Published | nominal interface | One declaration per new conforming type |

Prototype-to-production for interfaces is: delete one word, accept the quick-fixes. Cheap to move things around while coding; explicit exactly when it becomes API.

**Post-fold amendment (issue #490):** the "Published" row above is now partly enforced. `public duck interface` is a compile error — shape-matching may not cross a package boundary (`type.generics/DT1`). Package-internal duck interfaces stay legal in a published package and are reported by lint and at publish instead (DT2/DT3): DT1 already means no consumer can see them, so there's nothing a release gate would protect. This section's "harden it eventually" framing is now a rule where it protects someone else and a nudge where it doesn't.

### Renaming the `structural` keyword — decided: `duck interface`
...
| Candidate | Verdict |
|-----------|---------|
| `duck interface` | **Chosen.** The only candidate needing zero explanation; the unserious register doubles as the prototype-mode signal |
| `implied interface` | Runner-up — dignified literal pairing with *declared*; lost because dignity was the wrong goal: the keyword should mark looseness, not launder it |
| `matching interface` | Names the action, conjugates well in diagnostics — but teaches nothing by itself |
| `shape interface` | Names the criterion; same tier as `matching` |
| `automatching interface` | Fixes the auto-derive collision by fusing, but seven syllables next to `let`/`func`/`mut` |
| `auto trait` | Rejected: collides with **auto-derive** inside Rask itself (the core five are auto-derived *nominal* interfaces — "auto-derived but not auto" is a confusion factory), plus the unrelated Rust meaning |
| `lazy interface` | Rejected: "lazy" means deferred work everywhere else; nothing is deferred here |
| `magic interface` | Rejected: too cute, and the one word a transparency-first language can't use |
| `open interface` | Rejected: most overloaded word in PL (Kotlin `open`, open/closed principle, open unions) — invites confident misreading |
| `structural interface` | Retires to prose — docs say "known elsewhere as structural typing" for searchability |
...
No fine-grained `Add`/`Sub`/`Mul` interface zoo. A math type defines the methods it supports and gets exactly those operators.
...
| `spawn` sites | 52 | Sendability: compiler property, zero declarations ever needed — not an interface |
...
Rejected after survey: `Copy` (16-byte threshold, not an interface), `Sendable` (compiler-checked property), `Parseable` (not derivable), `Iterator` (retired by Sequence protocol), `Displayable` promotion (user-facing strings are intentional).
...
| Publish that helper | name the contract — a bound, or define the interface (IS2 assists) |
...
Next step: fold the accepted rules into generics.md / gradual-constraints.md / interfaces.md and retire this file.
