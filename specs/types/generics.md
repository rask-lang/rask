<!-- id: type.generics -->
<!-- status: decided -->
<!-- summary: Nominal interface conformance via extend...with, operator-to-method expansion, verified clone/equal/comparable, code specialization per type -->
<!-- depends: types/structs.md, types/enums.md, types/interfaces.md -->
...
| Rule | Description |
|------|-------------|
| **TD1: Module-scoped** | Interfaces must be module-scoped |
| **TD2: Default methods** | Interfaces may contain default implementations |
| **TD3: Composition** | Interfaces may compose using `:` syntax |
| **TD4: Members** | An interface body holds methods and associated types (`type Out`). Anything else — a `const`, a nested `struct` — is an error at the line that wrote it |
...
An interface can take type parameters, and the conformance says what they are:
...
## Duck Interfaces Are Package-Internal

`duck interface` is a sketching tool, and the compiler treats it as one. It is not a lighter-weight way to write an interface — it's a placeholder for a contract you haven't decided on yet, and it can't leave the package it was sketched in.
...
DT1 is the rule that carries the weight, and the reason is versioning. Structural conformance across a package boundary is a trap: adding a public method to your type could silently make it satisfy a duck interface in a package you've never read, and removing one could break code you've never seen. Neither shows up in your diff, and neither is something semver can describe. Keeping the interface package-internal means every type that matches it is a type the same author owns, so a shape change and its consequences land in the same review.

Inside a package, that hazard doesn't exist. Consumers can't see a private duck interface, can't name it, and can't have their types satisfy it. If you drop a method and one of your own types stops matching, that's a compile error in your own build — the same as any other internal break. So DT2 and DT3 tell you the sketch is still there and leave the decision with you. A published package whose internals are genuinely still in flux is a legitimate thing to ship; blocking it would be ceremony with no victim, which is the same line `type.gradual/GC11` draws for inferred private signatures.

```
ERROR [type.generics/DT1]: `duck interface` cannot be public
   |
4  |  public duck interface Frobber {
   |  ^^^^^^ ^^^^
   |
...
FIX: drop `duck` to make it a real interface (the compiler will list the types
...
| Rule | Description |
|------|-------------|
| **GF1: Public bounds explicit** | Public generic functions must declare interface constraints explicitly |
...
| Type has | Interface requires | Satisfied |
|----------|----------------|-----------|
...
For `duck interface`, the same signature check runs at the use site against the type's own methods — no declaration involved. Errors point at the declaration for nominal interfaces and at the use site for duck ones.
...
The core-interface family carries cross-interface contracts (`a == b` implies `hash(a) == hash(b)`; `compare` agrees with `eq`) that data structures physically rely on. Auto-derive keeps them consistent by construction; overrides must not silently break that.
...
| **OC2: Loud, with the fix** | Using a cancelled conformance is a compile error at the use site naming the override and the fix: declare the dependent interface consistent with the new eq |
...
`Debug` and `Error` are auto-derived too and are *not* carved out. Nothing is stored, keyed or transmitted on their say-so; a third-party `Debug` changes what a log line says, which is the ordinary ambiguity XC3 covers. The six are a closed list: an interface that turns out to decide what happens to someone else's data gets its own design round, not a quiet addition here.
...
XC1 empties that of consequence for the stdlib's contracts — a hash can't be third-party, so two packages keying the same type are keying it the same way. What's left is a user interface carrying a contract of its own: a user-written sorted container instantiated under two different orderings. XC5 keeps the instantiations apart so neither runs the other's code, but a value built by one and probed by the other still misbehaves, and no diagnostic in this design catches it.

That residual is the price of having no orphan rule, and it's bounded: it takes two packages that independently conform the same foreign type to the same foreign interface, *and* a value crossing between them. Rust charges every user a newtype wrapper to rule it out. Rask charges the one user who hits it, and charges the same wrapper (XC6) — one line, where it's actually needed.
...
**Third-party contract-interface conformance [XC1]:**
...
| **CC3: One condition per block** | A `where` clause applies to the whole block. Interfaces needing different conditions split into separate blocks |
...
This is the same conditionality auto-derive has always applied implicitly ("Vec of Cloneable is Cloneable" — CL1), with syntax for user interfaces. The clause only exists on generic types conforming to interfaces; concrete types never write it.

## Operator Expansion

The compiler expands operators into method calls before type checking (G4), then verifies the method exists.

| Operator | Desugars To | Interface Requirement |
|----------|-------------|-------------------|
...
| Struct with `any Interface` field | NOT Equal unless manually implemented |
| Struct with closure field | NOT Equal (closures have no equality) |

## Compiler-Verified Hashable

The compiler auto-derives Hashable where all fields implement Hashable. Since Hashable requires Equal (parent interface), auto-derive applies only when both are satisfied.
...
| Nominal newtype (`type Id = u64 implements …`) | Only when the clause lists `Hashable` — a newtype inherits nothing it doesn't name (`type.aliases/T11`) |

## Compiler-Verified Comparable

The compiler auto-derives Comparable where all fields implement Comparable — lexicographic by declaration order. Since Comparable requires Equal (parent interface), auto-derive applies only when both are satisfied.
...
## Must-Consume Types in Interfaces
...
Compiler collects all methods from the full parent interface chain, deduplicates identical requirements, errors on conflicts.

```rask
interface Hashable: Equal {
    func hash(self) -> u64
}

interface HashKey: Hashable + Cloneable {}
// Requires: eq (from Equal), hash (from Hashable), clone (from Cloneable)
```

## Code Specialization (Monomorphization)

When you call `sort<i32>` and `sort<string>`, the compiler generates two separate `sort` functions — one optimized for `i32`, one for `string` (G6). Interface matching is verified at each call site. No whole-program analysis.
...
| One package declares the same (type, interface) twice | XC3 | Compile error at the second declaration |
...
| Interface evolution | TD2 | Adding a required method with a default body is non-breaking; without one it breaks every conformer (major version) |
...
| More than 2 type params | — | Not in MVP; interfaces limited to 1-2 parameters |
...
**Default (removed):** Zero corpus usage, and DF-style universal zeros were Go zero-values by another name — a back door around Rask's all-fields-required construction. Declared field defaults replaced the interface; `Config {}` is the default value when every field declares one. No spec API used `T: Default` as a bound at removal time; a constructible-empty bound can return from usage evidence if ever needed.

**G4 (operator expansion):** Makes numeric code ergonomic — `a + b` reads naturally while the interface system handles dispatch.
...
**`duck interface`:** The opt-in that replaced `explicit interface` when the default flipped, renamed from `structural` (jargon). The register is deliberate — the keyword reading as unserious *is* the signal that the contract is loose by design. Sketch with it, delete the keyword to harden (the compiler generates the missing declarations). The stdlib ships zero duck interfaces; docs note the concept is known elsewhere as structural typing.
...
- **Type system**: Conformance declared and checked locally at the `extend` block; bounds checked at use site. The one thing that isn't local is the (type, interface) table XC3 reads to spot a second conformance — a lookup, not an analysis pass. `duck interface` checks shape at use site, and DT1 is a declaration-local visibility check
- **Build system**: `rask publish` scans the package for `duck interface` declarations and warns (DT2, `struct.build/PB8`) — a syntactic check over the package's own sources, no dependency analysis, no effect on whether the release proceeds
- **Concurrency**: Generic tasks can send owned generic values; interfaces verified per concrete type
...
- **Closures**: Generics in closures capture by value; interfaces verified at closure usage
- **Runtime polymorphism**: `any Interface` enables heterogeneous collections; see `type.interfaces`

### Standard Library Interfaces

| Interface | Methods | Auto-Derived? |
|-------|---------|---------------|
...
- [Interfaces](interfaces.md) — Interface definitions and `any Interface` polymorphism (`type.interfaces`)
