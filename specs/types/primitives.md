...
| `my_f64 + my_i64` | CV1a | Compile error (E0401) — same rule under an operator, either way round. An unsuffixed literal is not affected: it takes the slot's type, so `my_f64 + 1` is `my_f64 + 1.0` |
