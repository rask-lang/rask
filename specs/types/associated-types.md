...
<!-- status: decided -->
<!-- summary: A trait may name a type its conformance supplies; projections are read off a conformance, never searched for -->
<!-- depends: types/generics.md, types/traits.md -->
<!-- implemented-by: compiler/crates/rask-types/ -->

# Associated Types

A trait can name a type it doesn't know yet and let each conformance fill it in:
...
Every associated type here is a **lookup**. Given `Self` and the trait's arguments, there is one conformance, and `Out` is read off it. Nothing runs backwards — knowing `Out` never picks `Self`, and no constraint ties two projections together and asks the compiler to solve for both.
...
| Rule | Description |
|------|-------------|
| **AT1: Declared in the trait** | `type Out` in a trait body declares an associated type. It is a member of the trait like a method — part of what a conformance owes |
| **AT2: Supplied by the conformance** | `type Out = Meters` inside the `extend T with Trait` block. A conformance that leaves one unsupplied and undefaulted is an error at the block, naming the associated type |
| **AT3: Projection is `.`** | `Self.Out` inside the trait, `T.Out` in generic code where `T` carries the bound. A dot, like every other member access in Rask — not `::` |
| **AT4: Declared defaults** | `type Out = Self` in the *trait* gives a default; a conformance may then omit it. Without a default the conformance must state it |
...
| **AT8: One binding per applied trait** | The binding belongs to `(Self, trait with its arguments)`. `Mul<f64>` and `Mul<Meters>` are different conformances with different `Out`s, whether they're on one type or two |
...
<!-- test: skip -->
```rask
extend Meters with Mul<f64>    { type Out = Meters }
...
**Encoding.** `std.encoding` recorded wanting `Encode`/`Decode` as bounds and settling for marker traits because a `Serializer` hierarchy needs associated types. That's now writable. Whether to rewrite encoding around it is a separate call — the markers work and the rewrite is not free.

## What is deliberately left out

- **Equality constraints** (AT7). The search case.
- **Associated constants.** `const N: usize` in a trait body. No customer; comptime parameters cover the cases that came up.
...
### Why the default lives in the trait

`type Out = Self` on the declaration says "most conformances answer in their own type," once, where the trait is designed. The alternative — a per-feature rule like OR3's — says it again in every spec that wants it, and the two spellings drift. One rule, declared where the contract is.
