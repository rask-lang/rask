...
func dbg<T: Debug>(v: T, @call_text(v) text: string) -> T {
    print("{text} = {v:?}")
    return v
}
```
...
- `@call_text(param)` → `string`: source text of the argument expression for `param`.
...
**A. Default expression** — `a_text: string = @call_site.text(a)`. Its strength: a default
...
**B. Parameter annotation** (chosen, shown above) — `@call_text(a) a_text: string`.
...
- **Forwarding guarantees provenance, not correspondence.** With two text captures,
  ...
  which wrappers structurally can't satisfy. Honest cap: a captured `string` is genuinely
...
<!-- test: skip -->
```rask
func expect<T>(v: T?, msg: string, @call_location loc: SourceLoc) -> T {
...
string, a stored value — is a compile error. So captured facts are unforgeable: a `string`
...
to the const-representable set (primitives, `string`, enums and fixed arrays of these —
...
- **Lazy parameters** — hidden non-evaluation is hidden control flow. What the explicit
  ...
  `debug` overloads on `string` and `func() -> string`. Three extra characters buy visible
