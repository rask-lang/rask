...
| Orphan rule / newtype tax | Rust | Core four interfaces owner-only, everything else open; duplicate conformance is a loud use-site error | issue #312 (open) |
...
| Manual vtables, `@fieldParentPtr` UB | Zig | `any Interface` is a language-level fat pointer | `type.generics/G7` |
