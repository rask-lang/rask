...
- **Collections carry no eager adapters** (`SEQ41`). `Vec.map`/`filter`/`take`/`skip`/`fold`/`sum`/… are deleted; the chain is the spelling. The lazy ones that replace them aren't written out either — `rask-stdlib/src/forwarders.rs` generates them from `extend Sequence<T>` for any type that declares `as_sequence` (`SEQ49`), and the result is parsed as one more stub file.
