...
PM6c is PM4 and PM5 read the other way round. Those two make the marker
syntactic so that reading the call site is enough; that only pays off if the
marker is true, and `take` on an `i64` is the one that isn't. A missing marker
sends the reader to the signature. A wrong one doesn't send them anywhere —
they have already been told, incorrectly.

It is deliberately about the spelling and not about the type. A generic `take`
is where a Copy value legitimately meets one, and the declaration can't know:

```rask
func push(mutate self, take item: T)    // fine — T might be anything

let fee = 5
charges.push(fee)
charges.push(fee * 2)                   // `fee` is still there (PM6b)
```
