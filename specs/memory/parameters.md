...
| **PM6: A borrow can't be given away** | A `param: T` cannot be consumed inside the body — not by a `take` parameter, not by a `take self` method, and `own` at the inner call site changes nothing. The caller keeps the value and goes on using it, so consuming it would leave them holding something that's gone (`mem.linear/L1`). Compile error at the consumption, pointing at the declaration; `take` on the declaration is the fix. A `mutate` parameter is exclusive access rather than ownership, so it *may* be consumed — taking the value out and writing a replacement back is what the mode is for |
