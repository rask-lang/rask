...
| **PM6b: A Copy argument is copied, not given** | PM6 is about giving a value away, and a Copy type is never given away — handing one to a `take` parameter copies it and the caller keeps theirs. So `f(take n: i64)` called with a borrowed `n: i64` parameter is fine, and so is pushing an `i64` or a `Link<T>` into a `Vec` twice. The threshold is the ordinary one (`mem.ownership`): sixteen bytes, and never for a linear value |
