...
**Error handling.** Errors are values: `T or E` is a builtin sum type with type-based branch disambiguation (no `Ok`/`Err` wrappers), `try` for propagation, `??` fallback, `x!` force-unwrap. `T?` is sugar for `T or none` — the optional shape — sharing the same machinery. Error types compose with `A | B` union syntax. `E` must implement `ErrorMessage` (structural `message() -> string`); `none` is exempt as the absent sentinel. Disjointness rule (T ≠ E) makes construction unambiguous; newtype is the escape hatch. No exceptions, no hidden control flow. See [error-types.md](types/error-types.md), [optionals.md](types/optionals.md).
...
**Traits.** Nominal by default — conformance is declared with `extend Type with Trait`, because conformance is a semantic claim, not just a shape. `structural trait` opts into shape-matching for traits where the method bundle is the whole contract. Runtime polymorphism via `any Trait` for heterogeneous collections; conversion is explicit (`value as any Trait`) — it allocates, and the cast marks where. Conformance and generic constraints are in [generics.md](types/generics.md); `any Trait` runtime dispatch is in [traits.md](types/traits.md).
