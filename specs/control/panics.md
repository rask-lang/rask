...
| B | Panic while holding a lock | Release cleanly, no poisoning (LK1–LK2) | Rust-style poison-with-error-channel; sticky poison that panics later acquirers; abort |
...
Sticky poison-panic (next acquirer panics) was the strongest alternative — it converts possible data corruption into loud failure, which fits "fail loudly." Rejected because it cascades: one panicked request handler turns a shared metrics mutex into a landmine that kills every future task that touches it, and recovering requires a heal/clear API surface that admits panics into normal control flow. LK3 + the commit pattern is the honest version: the language guarantees its invariants, yours are your job.
