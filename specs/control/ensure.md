...
| **EN5: try forbidden in ensure** | Cannot use `try` inside an ensure body or its `or` handler |
...
Cleanup actions may fail. Errors are ignored by default; opt-in handling with an `or` clause.
...
FIX: Remove try and optionally handle errors with an or clause:
...
**Ensure on non-linear without an `or` clause [ER1]:**
```
...
**EN4 (errors ignored by default):** Most cleanup errors are unrecoverable. What do you do when `close()` fails? The OS has already released the resource. Silent ignore keeps simple cases simple (no ceremony). The `or` clause provides opt-in visibility for the rare cases where cleanup errors matter.
...
- Whether errors are handled (`or` clause present)
