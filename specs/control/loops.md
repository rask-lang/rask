...
| **LP18a: Body writes are captured for you** | The desugared closure captures every enclosing local the body writes, mutably, with no annotation (`type.sequence/SEQ40`, `mem.closures/MC5`). Accumulating in a sequence loop reads exactly like accumulating in a Vec loop |
