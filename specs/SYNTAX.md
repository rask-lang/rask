...
**Conformance is declared:** a type satisfies an interface through `Type implements Interface` (`type.generics/G1`):
```rask
...
If the type already has the methods, an empty declaration suffices: `Point implements Displayable {}`.
