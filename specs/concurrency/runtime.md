...
**No `Pin` required.** A task holds only owned values — a closure can't capture a borrow cross-task (`mem.closures/MC3`) and a link can't cross at all (`mem.ownership/T2`) — so nothing in a task points into a frame that could move. With stackful fibers the question is moot anyway: there is no state machine.
