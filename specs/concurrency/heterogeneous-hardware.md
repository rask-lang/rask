...
1. **O1 — the queue model (hardest).** Does a device expose one queue or many? Is a buffer bound to a queue or free-floating across them? Queues are *externally synchronized* — concurrent submission to one queue from two tasks is a driver-level race. This is where "device = async executor" earns its keep or leaks, and it will force the interface's real shape. Cross-task device access hangs on this.
