...
|------|--------|----------|
| **D4: Task scheduling** | Green task interleaving | Sim runs one task at a time; task switch order drawn from the seed. Adversarial schedules (pathological interleavings) are a seed away, not a fluke |
...
| **D8: Container slot allocation** | Which slot an insert takes, and a handle's generation | Deterministic function of the operation sequence. This is what a round trip preserves (`mem.relocatable/NP1`) and what a node's slot number records (`mem.racks/RK12`) |
...
| **D11: Addresses** | Pointer values leaking into logic | No address-of, and no operation yields an address as a number. `Link<T>` is the exception that shaped the rule: it *is* an address and it *is* storable (`mem.racks/RK2`), so ordering links is rejected outright rather than being impossible for want of a reference type — `<` and `sort` over links would answer from the allocator (`mem.racks/RK11`, E0406). A link used as a `Map` key hashes its node's slot number (`mem.racks/RK12`), not its address, so replay walks the buckets in the same order. Holds for Rask code only: linked C can hash or sort by a pointer freely, so sim gives the C side a fixed-base allocator (`sim/B8`) |
